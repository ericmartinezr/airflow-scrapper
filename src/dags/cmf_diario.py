...
INDICADORES = ["dolar", "euro", "uf"]
ENDPOINT = "https://api.cmfchile.cl/api-sbifv3/recursos_api/{indicador}/{year}/{month}/dias/{day}"


@dag(
    "cmf_diario",
    description="Scrapper DAG for CMF (diario)",
    schedule="@daily",
...
def scrapper():
    ...
    def extract_indicadores(indicador: str, **context):
        try:
            ...
            date = logical_date.format("YYYY-MM-DD").split("-")
            year, month, day = date
            ...
            indicador_map = {
                "dolar": "Dolares",
                "euro": "Euros",
                # UF se carga cada ciertos días en el mes, no se carga diariamente
                # pero se cargan varios dias a la vez al menos 2 veces en el mes
                # asi que lo tratare como diario
                "uf": "UFs"
            }
